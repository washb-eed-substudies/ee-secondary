...
sens_lm_N1<-cbind("sens_lm","T1", rownames(sens_lm_t1_mn), as.data.frame(sens_lm_t1_mn)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6")) 
l_N1<-cbind("sens_lact","T1", rownames(sens_lac_t1_mn), as.data.frame(sens_lac_t1_mn)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6")) 
m_N1<-cbind("sens_mann","T1", rownames(sens_man_t1_mn), as.data.frame(sens_man_t1_mn)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6")) 
...
sens_lm_N2<-cbind("sens_lm","T2", rownames(sens_lm_t2_mn), as.data.frame(sens_lm_t2_mn)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6")) 
reg_N2<-cbind("REG","T2", rownames(reg1b2_t2_mn), as.data.frame(reg1b2_t2_mn)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6")) 
l_N2<-cbind("sens_lact","T2", rownames(sens_lac_t2_mn), as.data.frame(sens_lac_t2_mn)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6")) 
m_N2<-cbind("sens_mann","T2", rownames(sens_man_t2_mn), as.data.frame(sens_man_t2_mn)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6")) 
...
sens_lm_N3<-cbind("sens_lm","T3", rownames(sens_lm_t3_mn), as.data.frame(sens_lm_t3_mn)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6")) 
l_N3<-cbind("sens_lact","T3", rownames(sens_lac_t3_mn), as.data.frame(sens_lac_t3_mn)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6")) 
m_N3<-cbind("sens_mann","T3", rownames(sens_man_t3_mn), as.data.frame(sens_man_t3_mn)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6")) 
...
sens_lm_dif1<-cbind("sens_lm","T1", rownames(sens_lm_t1_unadj_M), as.data.frame(sens_lm_t1_unadj_M)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6","7","8","9")) 
l_dif1<-cbind("sens_lact","T1", rownames(sens_lac_t1_unadj_M), as.data.frame(sens_lac_t1_unadj_M)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6","7","8","9")) 
m_dif1<-cbind("sens_mann","T1", rownames(sens_man_t1_unadj_M), as.data.frame(sens_man_t1_unadj_M)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6","7","8","9")) 
...
sens_lm_dif2<-cbind("sens_lm","T2", rownames(sens_lm_t2_unadj_M), as.data.frame(sens_lm_t2_unadj_M)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6","7","8","9")) 
...
l_dif2<-cbind("sens_lact","T2", rownames(sens_lac_t2_unadj_M), as.data.frame(sens_lac_t2_unadj_M)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6","7","8","9")) 
m_dif2<-cbind("sens_mann","T2", rownames(sens_man_t2_unadj_M), as.data.frame(sens_man_t2_unadj_M)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6","7","8","9")) 
...
sens_lm_dif3<-cbind("sens_lm","T3", rownames(sens_lm_t3_unadj_M), as.data.frame(sens_lm_t3_unadj_M)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6","7","8","9")) 
l_dif3<-cbind("sens_lact","T3", rownames(sens_lac_t3_unadj_M), as.data.frame(sens_lac_t3_unadj_M)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6","7","8","9")) 
m_dif3<-cbind("sens_mann","T3", rownames(sens_man_t3_unadj_M), as.data.frame(sens_man_t3_unadj_M)) %>% `rownames<-`(NULL) %>% setNames(., c("1","2","3","4","5","6","7","8","9")) 
...
sens_lact<-dif_df[dif_df$Location=="sens_lact",]
sens_mann<-dif_df[dif_df$Location=="sens_mann",]
sens_lm<-dif_df[dif_df$Location=="sens_lm",]
...
c(min(sens_lact$lower.ci),max(sens_lact$upper.ci))
c(min(sens_mann$lower.ci),max(sens_mann$upper.ci))
c(min(sens_lm$lower.ci),max(sens_lm$upper.ci))
...
long.diff.plot<-function(d, yrange=c(-1.5,0.5),  cols=cols, tickspace=0.2){
  ...
  if((d$Location=="sens_lact")[1]){yrange<-yrange
    ytics <- seq(-1.5,0.5,by=0.2)
    title<-"Ln sens_lactulose (mmol/L)"}
  if((d$Location=="sens_mann")[1]){yrange<-yrange
    ytics <- seq(-1.5,0.5,by=0.2)
    title<-"Ln sens_mannitol (mmol/L)"}
  if((d$Location=="sens_lm")[1]){yrange<-yrange
    ytics <- seq(-1.5,0.5,by=0.2)
    title<-"Ln L:M ratio"}
...
pdf("EE-mean-longitudinal-difference-plots-sensitivity.pdf",width=10,height=8.5, paper="USr")
...
long.diff.plot(d<-dif_df[dif_df$Location=="sens_lact",], yrange=c(-1.3,0.7),  cols=cols, tickspace=0.2)
legendplot(F,1)
long.diff.plot(d<-dif_df[dif_df$Location=="NEO",], yrange=c(-1.3,0.7),  cols=cols, tickspace=0.2)
long.diff.plot(d<-dif_df[dif_df$Location=="sens_mann",], yrange=c(-1.3,0.7),  cols=cols, tickspace=0.2)
legendplot(x=0.5, y=.5, T, 1.3)
long.diff.plot(d<-dif_df[dif_df$Location=="MPO",], yrange=c(-1.3,0.7),  cols=cols, tickspace=0.2)
long.diff.plot(d<-dif_df[dif_df$Location=="sens_lm",], yrange=c(-1.3,0.7),  cols=cols, tickspace=0.2)
legendplot(F,1)
